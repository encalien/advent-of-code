...
let maxAdd = 2020 - expenses[0] - expenses[1];
expenses = expenses.filter(expense => expense <= maxAdd);
    ...
    for (const num2 of expenses) {
      if (num !== num2) {
        for (let i = 1; i < expenses.length; i++) {
          let sum = num + num2 + expenses[expenses.length - i];
          if (sum < 2020) break;
          if (sum == 2020) return [num, num2, expenses[expenses.length - i]];
        }
      }
    }
  }
...
let theNumbers = find2020();
let result = theNumbers[0] * theNumbers[1] * theNumbers[2];
