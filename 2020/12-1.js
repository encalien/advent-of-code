...
for (const step of stepList) {
  let isTurn = step.direction === "L" || step.direction === "R";

  if (isTurn) {
    changeDirection(step);
  } else {
    moveShip(step.direction, step.amount);
  }
}

function changeDirection(step) {
  let changeCount = step.amount / 90;
  let index = directionList.indexOf(currentDirection);
  ...
  console.log("Previous direction", currentDirection);
  console.log("Changing to", step.direction, "for", step.amount, "degrees");
  currentDirection = directionList[index];
  console.log("Current direction", currentDirection);
  console.log("=====================");
}

function moveShip(direction, amount) {
  switch (direction) {
    case "N":
      northSouth += amount;
      break;
    ...
    case "W":
      ...
      break;
    case "F":
      moveShip(currentDirection, amount);
      break;
  }
  if (direction !== "F") {
    console.log("ship moved in direction", direction, "for", amount);
