...
function execute(bootCodeClone) {
  ...
  switch (bootCodeClone[index].operation) {
    ...
    case "nop":
      ...
      break;
    case "jmp":
      index += bootCodeClone[index].argument;
      break;
  }
}


function test(bootCodeClone) {
  while (!bootCodeClone[index].executed) {
    execute(bootCodeClone)
    if (index === bootCodeClone.length - 1) return acc;
  }
}

for (let i = 0; i < bootCode.length; i++) {
  let bootCodeClone = JSON.parse(JSON.stringify(bootCode))
  index = 0;
  acc = 0;

  if (bootCodeClone[i].operation === "jmp") {
    bootCodeClone[i].operation = "nop";
    // console.log("on index " + i + " changed jmp into nop");
  } else if (bootCodeClone[i].operation === "nop") {
    bootCodeClone[i].operation = "jmp";
    // console.log("on index " + i + " changed nop into jmp");
  }
  if (test(bootCodeClone)) {
