...
def get_line_vent_positions(line):
    ...
    decreasing = (x_fixed and line[0][1] > line[1][1]) or (
        y_fixed and line[0][0] > line[1][0])
    pos_range = None

    if x_fixed:
        range_end = line[1][1] + (-1 if decreasing else 1)
        pos_range = range(line[0][1], range_end, -1 if decreasing else 1)
    if y_fixed:
        range_end = line[1][0] + (-1 if decreasing else 1)
        pos_range = range(line[0][0], range_end, -1 if decreasing else 1)
    if not pos_range:
        return positions
