      ...
      if (!(px === x && py === y)) {
        ...
        if (seat && seat.current === ".") {
          ...
          if (isVertical) {
            let mod = py < y ? -1 : 1;
            let counter = 2;
            while (seat && seat.current === ".") {
              newY = y + (counter * mod);
              seat = seatingLayout[newY] && seatingLayout[newY][px];
              counter++;
            }
          }
          if (isHorizontal) {
            ...
            while (seat && seat.current === ".") {
              newX = x + (counter * mod);
              seat = seatingLayout[py] && seatingLayout[py][newX];
              counter++;
            }
          }
          if (isDiagonal) {
            let modX = (px - x) / Math.abs((px - x));
            let modY = (py - y) / Math.abs((py - y));
            let counter = 2;
            while (seat && seat.current === ".") {
              newX = x + (counter * modX);
              newY = y + (counter * modY);
              seat = seatingLayout[newY] && seatingLayout[newY][newX];
              counter++;
            }

          }
        }
        let fx = newX !== undefined ? newX : px;
        let fy = newY !== undefined ? newY : py;
        if (seat) adjacentSeats.push([fx, fy]);
      }
    }
  }
...
seatingLayout = seatingLayout.map((row, y) => row.map((seat, x) => {
  seat.adjacentSeatPositions = seat.current === "." ? [] : determineRelevantPositions(x, y);
  return seat;
}))
...
while (modified) {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < columns; x++) {
      let seat = seatingLayout[y][x];
      if (seat.current === "L") {
        let hasAnyAdjacentFull = false;
        for (const sp of seat.adjacentSeatPositions) {
          if (seatingLayout[sp[1]][sp[0]].current === "#") hasAnyAdjacentFull = true;
        }
        if (!hasAnyAdjacentFull) {
          seatingLayout[y][x].next = "#";
          modified = true;
        }
      } else if (seat.current === "#" && seat.adjacentSeatPositions.length > 4) {
        let adjacentFullSeatsCount = 0;
        for (const sp of seat.adjacentSeatPositions) {
          if (seatingLayout[sp[1]][sp[0]].current === "#") adjacentFullSeatsCount++;
        }
        if (adjacentFullSeatsCount >= 5) {
          seatingLayout[y][x].next = "L";
          modified = true;
        }
      }
    }
  }
  seatingLayout = seatingLayout.map(row => row.map(seat => {
    seat.current = seat.next;
    return seat;
  }));
  pass++;
  if (pass > 200) break;
...
let flatLayout = seatingLayout.flat().map(el => el.current);
let fullSeatCount = flatLayout.filter(el => el === "#").length;
