  ...
  for (let py = y - 1; py <= y + 1; py++) {
    for (let px = x - 1; px <= x + 1; px++) {
      if (!(px === x && py === y)) {
        ...
        if (seat) adjacentSeats.push(seat);
      }
    }
  }
...
while (modified) {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < columns; x++) {
      ...
      if (seat.current === "L") {
        let hasAnyAdjacentFull = determineRelevantPositions(x, y).find(seat => seat.current === "#");
        if (!hasAnyAdjacentFull) {
          seatingLayout[y][x].next = "#";
          modified = true;
        }
      } else if (seat.current === "#") {
        let adjacentFullSeats = determineRelevantPositions(x, y).filter(seat => seat.current === "#");
        if (adjacentFullSeats.length >= 4) {
  ...
  seatingLayout = seatingLayout.map(row => row.map(seat => { return { current: seat.next, next: seat.next } }));
  pass++;
  if (pass > 100) break;
}
